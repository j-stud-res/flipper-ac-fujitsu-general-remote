...
static const char* g_SleepTimeValueLabels[] = {
  "30min", "1h", "2h", "3h", "5h", "7h", "9h"
};

static const uint32_t g_SleeptimeValuesInMinutes[] = {
  30, 60, 120, 180, 300, 420, 540
};
...
void view_fujitsu_remote_page_sleep_enter_callback(void* ctx, uint32_t index)
{
  ...
  if(index == FujiSleepListItemName_Confirm)
  {
    ...
    view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_SleepChanged);
...
void view_fujitsu_remote_page_sleep_alloc(FujitsuRemoteApp* app)
{
  ...
  variable_item_list_set_enter_callback(
    ...
    view_fujitsu_remote_page_sleep_enter_callback,
    app
  );  
  ...
  variable_item_list_set_selected_item(app->variable_item_list_sleep, FujiSleepListItemName_Timer);
