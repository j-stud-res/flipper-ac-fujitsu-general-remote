...
{
  if(cur_button == FujiPage3Button_Timer)
    return FujiPage3Button_Power;
...
{
  if(cur_button == FujiPage3Button_Power)
    return FujiPage3Button_Timer;
...
static void view_fujitsu_remote_page3_timer1_callback(void* ctx)
{
  ...
  with_view_model(app->view_page3, FujitsuRemotePage3Model* model, {
...
void view_fujitsu_remote_page3_alloc(FujitsuRemoteApp* app)
{
  ...
  /* Init model data*/
  with_view_model(view, FujitsuRemotePage3Model * model, {
    model->button_selected = g_FujiPage3DefaultButton;
    model->cancel_in_progress = false;
    model->power_in_progress = false;
  }, false);

  app->timers->view_page3_timer1 = furi_timer_alloc(view_fujitsu_remote_page3_timer1_callback, FuriTimerTypeOnce, app);
  app->timers->view_page3_timer2 = furi_timer_alloc(view_fujitsu_remote_page3_timer2_callback, FuriTimerTypeOnce, app);
}
...
void view_fujitsu_remote_page3_draw_callback(Canvas* canvas, void* model)
{
  ...
  { /* ROW 1 Timer */
    if(m->button_selected == FujiPage3Button_Timer)
  ...
  { /* ROW 2 Cancel */
    if(m->button_selected == FujiPage3Button_Cancel)
      canvas_invert_color(canvas);
  ...
  { /* ROW 3 Sleep */
    if(m->button_selected == FujiPage3Button_Sleep)
      canvas_invert_color(canvas);
  ...
  { /* ROW 4 Power */
    if(m->button_selected == FujiPage3Button_Power)
      canvas_invert_color(canvas);
...
bool view_fujitsu_remote_page3_input_callback(InputEvent* event, void* ctx)
{
  ...
  with_view_model(app->view_page3, FujitsuRemotePage3Model * model, {
    switch (event->key) {
      case InputKeyLeft:
        view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_OpenPage2);
        break;
      case InputKeyRight:
        view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_OpenPage1);
        break;
      case InputKeyUp:
        model->button_selected = view_fujitsu_remote_page3_button_up(model->button_selected);
        update = true;
        break;
      case InputKeyDown:
        model->button_selected = view_fujitsu_remote_page3_button_down(model->button_selected);
        update = true;
        break;
      case InputKeyOk:
        switch(model->button_selected) {
          case FujiPage3Button_Timer:
            view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_OpenPageTimer);
            break;
          case FujiPage3Button_Cancel:
            ...
            view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_CancelTimer);
            update = true;
            break;
          case FujiPage3Button_Sleep:
            view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_OpenPageSleep);
            break;
          case FujiPage3Button_Power:
            ...
            view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_PowerModeToggle);
            update = true;
          default:
...
void view_fujitsu_remote_page3_enter_callback(void* ctx)
{
  ...
  FujitsuRemoteApp* app = ctx;
  with_view_model(
    app->view_page3, FujitsuRemotePage3Model * model, {
      model->button_selected = g_FujiPage3DefaultButton;
      model->cancel_in_progress = false;
      model->power_in_progress = false;
    }, true);
