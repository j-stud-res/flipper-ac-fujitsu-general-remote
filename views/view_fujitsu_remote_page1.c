...
void view_fujitsu_remote_page1_alloc(FujitsuRemoteApp* app)
{
  ...
  /* Init model data*/
  with_view_model(view, FujitsuRemotePage1Model * model, {
    model->mode = ac->mode;
...
{
  view_dispatcher_add_view(app->view_dispatcher, FujitsuViewId_Page1_MainScreen, app->view_page1);
}
...
bool view_fujitsu_remote_page1_input_callback(InputEvent* event, void* ctx)
{
  ...
  if(event->type == InputTypeRelease && (event->key == InputKeyUp || event->key == InputKeyDown))
  {
    skip_temp_change = false;
    view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_TempChangeEnd);
    return true;
  }

  with_view_model(app->view_page1, FujitsuRemotePage1Model * model, {
    consumed = true;
    ...
    switch (event->key) {
        ...
        switch(event->type) {
          ...
          case InputTypeShort:
            if(!skip_temp_change)
            {
              ...
              update = true;
              view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_TempUp);
            }
            break;
          default:
            consumed = false;
        ...
        switch(event->type) {
          ...
          case InputTypeShort:
            if(!skip_temp_change)
            {
              ...
              update = true;
              view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_TempDown);
            }
            break;
          default:
            consumed = false;
      ...
      case InputKeyOk:
        switch(event->type) {
          case InputTypePress:
            view_dispatcher_send_custom_event(app->view_dispatcher, FujitsuRemoteAppEvent_PowerToggle);
